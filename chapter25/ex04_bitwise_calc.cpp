...
//
// Chapter 25 Exercise 4 - Add Bitwise operations to the Ch 7 Calcultor
//
// According to a few sources online, bitwise operations have lesser precedence
// than addition and subtraction. Bitwise is added "above" expression, a
// statement() calls bitwise() which calls expression().
//
...
double calc_pow()
{
    ...
    t = ts.get();
    if (t.kind != ',') error("',' expected");
    int power = narrow_cast<int>(bitwise());
    t = ts.get();
    if (t.kind != ')') error("')' expected");
